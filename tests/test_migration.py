...
def test_migration_script_runs():
    ...
    result = subprocess.run(['python', 'db_migration/migrate.py'], capture_output=True, text=True)

    # Assert that the script exits with a success code (0)
    assert result.returncode == 0

    # Assert that the expected success message is present in the stderr (logging output)
